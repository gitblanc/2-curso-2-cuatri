...
void Processor_InitializeInterruptVectorTable(int interruptVectorInitialAddress)
{
	...
	interruptVectorTable[CLOCKINT_BIT] = interruptVectorInitialAddress + 4;	 //CLOCKINT_BIT = 8 EJ2 V2
}
...
void Processor_DecodeAndExecuteInstruction()
{
	...
	case DIV_INST:
		if (operand2 == 0)
			...
			Processor_RaiseException(DIVISIONBYZERO);//EJ 1 c) V4
		else
		{
			registerAccumulator_CPU = operand1 / operand2;
			registerPC_CPU++;
	...
	case HALT_INST:
		...
		{
			//Processor_RaiseInterrupt(EXCEPTION_BIT);
			Processor_RaiseException(INVALIDPROCESSORMODE);//EJ 1 c) V4
		}
		break;
	// ej 16 v1 END
	...
	case OS_INST: // Make a operating system routine in entry point indicated by operand1
		...
		if (Processor_PSW_BitState(EXECUTION_MODE_BIT))
		{
			int procesoEj = OperatingSystem_GetExecutingProcessID();
			ComputerSystem_DebugMessage(130, HARDWARE, InstructionNames[operationCode], operand1, operand2, procesoEj, registerPC_CPU, registerAccumulator_CPU, registerPSW_CPU, Processor_ShowPSW());
	...
	case MEMADD_INST:
		...
		registerAccumulator_CPU = operand2 + operand1;
		registerPC_CPU++;
		break;
	// V1-ex0-end
	...
	// Unknown instruction
	default:
		Processor_RaiseException(INVALIDINSTRUCTION);//Ej 3 V4
