	...
	switch (registerCTRL_MMU) {
  	case CTRLREAD:
			if (Processor_PSW_BitState(EXECUTION_MODE_BIT)){ // Protected mode
				if (registerMAR_MMU < MAINMEMORYSIZE && registerMAR_MMU >= 0){//Ej 1 d) V4
					...
					Processor_RaiseException(INVALIDADDRESS);//Ej 1 d) V4
				}
			}
			else // Non-Protected mode
				if (registerMAR_MMU<registerLimit_MMU && registerMAR_MMU >= 0) {//Ej 1 V4
