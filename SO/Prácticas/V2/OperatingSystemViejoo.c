...
int OperatingSystem_ExtractFromReadyToRun(int); // V1-ex11
...
char * statesNames[5]={"NEW","READY","EXECUTING","BLOCKED","EXIT"}; // V1-ex10
...
int OperatingSystem_CreateProcess(int indexOfExecutableProgram) {
	...
	// V1-ex7-begin
	loadResult = OperatingSystem_LoadProgram(programFile, loadingPhysicalAddress, processSize);
...
void OperatingSystem_PCBInitialization(int PID, int initialPhysicalAddress, int processSize, int priority, int processPLIndex) {
	...
	ComputerSystem_DebugMessage(111,SYSPROC,PID,programList[processPLIndex]->executableName,statesNames[processTable[PID].state]); // V1-ex10
	...
	processTable[PID].queueId=programList[processPLIndex]->type;  // V1-ex11
}
...
void OperatingSystem_MoveToTheREADYState(int PID) {
	int previousState; // V1-ex10

	if (Heap_add(PID, readyToRunQueue[processTable[PID].queueId],QUEUE_PRIORITY ,&numberOfReadyToRunProcesses[processTable[PID].queueId] ,PROCESSTABLEMAXSIZE)>=0) { // V1-ex11
		// V1-ex10-begin
		previousState=processTable[PID].state;
		processTable[PID].state=READY;
		ComputerSystem_DebugMessage(110,SYSPROC,PID,programList[processTable[PID].programListIndex]->executableName,statesNames[previousState],statesNames[processTable[PID].state]);
		// processTable[PID].state=READY;
		// V1-ex10-end
	} 
...
int OperatingSystem_ShortTermScheduler() {
	...
	// V1-ex11: int selectedProcess;
	int selectedProcess=NOPROCESS; // V1-ex11
	int i; // V1-ex11

	// V1-ex11: selectedProcess=OperatingSystem_ExtractFromReadyToRun();
	// V1-ex11-begin
...
// V1-ex11: int OperatingSystem_ExtractFromReadyToRun() {
int OperatingSystem_ExtractFromReadyToRun(int queueId) { // V1-ex11  
	int selectedProcess=NOPROCESS;

	// V1-ex11: selectedProcess=Heap_poll(readyToRunQueue,QUEUE_PRIORITY ,&numberOfReadyToRunProcesses);
	selectedProcess=Heap_poll(readyToRunQueue[queueId],QUEUE_PRIORITY ,&numberOfReadyToRunProcesses[queueId]); // V1-ex11
...
void OperatingSystem_TerminateProcess() {
	...
	ComputerSystem_DebugMessage(110,SYSPROC,executingProcessID,programList[processTable[executingProcessID].programListIndex]->executableName,statesNames[previousState],statesNames[processTable[executingProcessID].state]);
	// processTable[executingProcessID].state=EXIT;
	// V1-ex10-end
...
void OperatingSystem_HandleSystemCall() {
	...
	int oldExecutingProcessID; //V1-ex12	
	int selectedProcess; //V1-ex12
	int queueId; //V1-ex12

	// Register A contains the identifier of the issued system call
	systemCallID=Processor_GetRegisterA();
	oldExecutingProcessID=executingProcessID; // V1-ex12
