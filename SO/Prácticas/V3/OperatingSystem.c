...
void OperatingSystem_PrintReadyToRunQueue()
{
	...
	for (i = 0; i < numberOfReadyToRunProcesses[1] - 1; i++)
	{
		// EJ 1 V2
		// readytorunqueue[i] -> es el pid del proceso
		ComputerSystem_DebugMessage(107, SHORTTERMSCHEDULE, readyToRunQueue[1][i].info, processTable[readyToRunQueue[1][i].info].priority);
	}
	if(numberOfReadyToRunProcesses[1] > 0){
		// EJ 1 V2
		ComputerSystem_DebugMessage(108, SHORTTERMSCHEDULE, readyToRunQueue[1][i].info, processTable[readyToRunQueue[1][i].info].priority);
	}
}
// EJ 11 V1
...
{
	OperatingSystem_ShowTime(SHORTTERMSCHEDULE);
	ComputerSystem_DebugMessage(121, SHORTTERMSCHEDULE, executingProcessID, programList[processTable[executingProcessID].programListIndex]->executableName, pidProceso, programList[processTable[pidProceso].programListIndex]->executableName);
