...
void OperatingSystem_HandleIOEndInterrupt(); // V5-Ex1
void OperatingSystem_IOScheduler(); // V5-Ex2
void OperatingSystem_DeviceControlerStartIOOperation(); // V5-Ex2
...
// V5-Ex2 IORb queue, implemented as a FIFO.
int IOWaitingProcessesQueue[PROCESSTABLEMAXSIZE]; // Queue of waiting processes.
int numberOfIOWaitingProcesses=0; // The number of waiting processes that it contains.
...
void OperatingSystem_InterruptLogic(int entryPoint) {
	switch (entryPoint) {
		...
		case CLOCKINT_BIT: // EXCEPTION_BIT=9
			...
			break;
		case IOEND_BIT: // EXCEPTION_BIT=8 When an input output interrupt is raised.
			OperatingSystem_HandleIOEndInterrupt(); // Call to the corresponding function.
			break;
	}
...
void OperatingSystem_HandleIOEndInterrupt() {
	...
	// To be implemented with future exercises...

} // END OperatingSystem_HandleIOEndInterrupt.

// Device Independent Handler. It adds a new IORB to the queue of the device
// and to notify the device dependent handler of this fact.
void OperatingSystem_IOScheduler() {

	// Add new IORB to the queue.
	QueueFIFO_add(executingProcessID, IOWaitingProcessesQueue, &numberOfIOWaitingProcesses, PROCESSTABLEMAXSIZE);

	// Mut notify the device dependent handler of the new IORB.
	OperatingSystem_DeviceControlerStartIOOperation();

} // END OperatingSystem_IOScheduler.

//
void OperatingSystem_DeviceControlerStartIOOperation() {

} // END OperatingSystem_DeviceControlerStartIOOperation.
