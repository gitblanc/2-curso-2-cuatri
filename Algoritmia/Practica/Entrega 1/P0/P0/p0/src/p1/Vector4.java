...
public class Vector4 {
	...
	public static void main(String arg[]) {
		...
		for (int i = 10; i < 143489071; i *= 3) { //con la i = 143489071 el programa peta(es el ultimo que puede ejecutar)
			...
			if(resultadoSumaConRellena <= 0) {
				iterarPequeÒos(v);
				System.out.println("PEQUE—O CON RELLENA---");
			}
			
			if(resultadoSumaSinRellena <= 0) {
				iterarPequeÒos(v);
				System.out.println("---PEQUE—O SIN RELLENA---");
			}
	...
	private static void iterarPequeÒos(int[] v2) {
		...
		long finTiempoS = System.currentTimeMillis();
		long res = (finTiempoS-inicioTiempoS);
		System.out.println("TARD” LA PEQ: " + res + " picosegundos");
