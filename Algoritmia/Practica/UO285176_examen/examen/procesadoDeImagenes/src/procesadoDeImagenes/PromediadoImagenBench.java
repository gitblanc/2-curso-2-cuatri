...
//		img_avger.saveResults(OUT_DIR_B);
		...
		//-------------EXAMEN--------------
		System.out.print("BACKTRACKING SIN BALANCEO:\n");
		System.out.println("TAM\tTIEMPO\tZNCC");
		for (int tam = 2; tam < 9; tam++) {
			n_bad = (int) ((PORCENTAJE_BAD / 100.) * tam);
			n_real = tam - n_bad;
			img_avger = new PromediadoImagen(REAL_IMG, BAD_IMG, n_real, n_bad, S_NOISE);
			t1 = System.currentTimeMillis();
		...
		System.out.print("BACKTRACKING SIN BALANCEO EXAMEN:\n");
		System.out.println("TAM\tTIEMPO\tZNCC");
		for (int tam = 2; tam < 9; tam++) {
			n_bad = (int) ((PORCENTAJE_BAD / 100.) * tam);
			n_real = tam - n_bad;
			...
			t1 = System.currentTimeMillis();
			for (int i = 0; i < nVeces; i++) {
				img_avger.splitSubsetsBacktrackingExamen();
			}
			t2 = System.currentTimeMillis();
			System.out.println(tam + "\t" + (t2 - t1) + "\t" + img_avger.zncc());
		}
...
//			t1 = System.currentTimeMillis();
//			for (int i = 0; i < nVeces; i++) {
//				//img_avger.splitSubsetsGreedyConBalanceoExamen(tam, 3,0,0,0);
//				img_avger.splitSubsetsGreedyConBalanceoExamen(tam, 3);// tam = num imágenes
//			}
//			t2 = System.currentTimeMillis();
//			System.out.println(tam + "\t" + (t2 - t1) + "\t" + img_avger.zncc());
//		}
